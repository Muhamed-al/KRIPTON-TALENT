...
public class CandidateService implements ICandidateService{
	public CandidateService (@Value ("${uploadDirectory}")String UPLOAD_DIR, CandidateEntityRepository candidateRepository, QualificationClient qualificationClient, ModelMapper mapper, KafkaTemplate<String, String> kafkaTemplate, JobClient jobClient, RestTemplate restTemplate) {
    ...
    public CandidateDetailsDto saveCandidateWithDetails(CandidateDetailsDto candidateDetailsDto ,String idUser, MultipartFile image) throws IOException, IllegalAccessException {
        ...
        if (image != null){
            ...
            candidateEntityBeforeSave.setImagePath(imagePath.substring(UPLOAD_DIR.indexOf("assets")));
        }else {
    ...
    @Transactional
    @Override
    public CandidateDetailsDto updateCandidateWithDetails(CandidateDetailsDto candidateDetailsDto, Long idCandidate,MultipartFile image) throws IllegalAccessException, IOException {
        CandidateEntity candidateEntityBeforeSave = mapper.map(candidateDetailsDto, CandidateEntity.class);
...
//        int random_int = (int)Math.floor(Math.random() * (10000 - 1 + 1) + 1);
    ...
    public ResponseEntity<?> createCandidateFromResume(MultipartFile file) throws IOException {
        ...
        ObjectMapper objectMapper = new ObjectMapper();
        CandidateDto candidate = objectMapper.readValue(response.getBody(), CandidateDto.class);
        ...
        try {
           ...
           // newCandidate.setSkills((Set<String>) candidate.getSkills());
            newCandidate.setSkills(new HashSet<String>(candidate.getSkills()));

...
def recommend_jobs(candidate_id,number_of_jobs):
    ...
    jobs_df = pd.DataFrame(jobs,columns=['id','title','company','description','status','employment_type','experience_level','image_path','skills'])
    ...
    for index, row in top_resumes_df.iterrows():
        ...
        jobs_list.append(f"{job_id},{title},{company},{imagePath},{description},{status},{employment_type},{experience_level},{match_percentage}")
    ...
    for i in range(len(jobs_list)):
        ...
        pro_key = ['id', 'title','company','imagePath','description','status', 'employment_type', 'experience_level', 'score', 'skills']
